...
export default function NovoPlanejamentoPage() {
  const [form, setForm] = useState({ disciplina:'', serie:'', bimestre:'1', conteudo:'', orientacoes:'', numObjetivos:'3' })
  ...
  const handleSalvar = async () => {
    ...
    const { data, error } = await supabase.from('planos_de_aula').insert({
      ...
      disciplina: form.disciplina, serie: form.serie, bimestre: parseInt(form.bimestre),
      conteudo: form.conteudo, habilidades_bncc: plano.habilidades_bncc, objetivos: plano.objetivos,
      desenvolvimento: plano.desenvolvimento, conclusao: plano.conclusao, dinamica: plano.dinamica, status: 'concluido'
    }).select().single()
          ...
          <form onSubmit={handleGerar} className="space-y-4">
            ...
            <div>
              ...
              <textarea name="orientacoes" value={form.orientacoes} onChange={handleChange} rows={3}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Ex: Incluir atividade prática, usar recursos visuais..." />
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              ...
              <ul className="space-y-2">
                {plano.objetivos.map((obj, i) => (
                  <li key={i} className="flex items-start gap-2 text-gray-700"><span className="text-green-500 mt-0.5">✓</span>{obj}</li>
                ))}
              </ul>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
